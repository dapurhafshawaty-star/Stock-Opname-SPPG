...
import { Database, Users, Trash2, UserPlus, RefreshCw, Download, FileText, Check, AlertCircle, Info, Cloud, ShieldCheck, Sparkles, Upload, Image as ImageIcon, X, Edit2 } from 'lucide-react';
...
export default function Settings({
  ...
  // Editing staff modal states
  const [editingStaff, setEditingStaff] = useState<UserProfile | null>(null);
  const [editName, setEditName] = useState('');
  const [editEmail, setEditEmail] = useState('');
  const [editRole, setEditRole] = useState<UserRole>('STAF_DAPUR');
  const [editPin, setEditPin] = useState('');

  const handleStartEditStaff = (profile: UserProfile) => {
  ...
  const handleUpdateStaffSubmit = async (e: React.FormEvent) => {
    ...
    if (!editName.trim() || !editEmail.trim() || editPin.length !== 4) {
      alert('Harap lengkapi nama, email, dan 4 digit PIN.');
      return;
    }
    ...
    try {
      ...
      await onAddStaffProfile(updatedProfile);
      setEditingStaff(null);
      alert(`Hak akses dan profil staff "${updatedProfile.name}" berhasil diperbarui di Google Cloud!`);
    } catch (err: any) {
      console.error(err);
      alert('Gagal mengedit staff: ' + err.message);
    } finally {
  ...
  return (
    <div className="space-y-6 font-sans text-slate-800">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 border-b border-slate-100 text-slate-500 font-bold uppercase text-[10px]">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right">Kelola Hak Akses Email</th>
                  </tr>
                </thead>
                  ...
                  {staffProfiles.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-600 font-mono text-[11px]">
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-slate-100 border border-slate-200/60 font-semibold text-slate-700">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            type="button"
                            onClick={() => handleStartEditStaff(p)}
                            disabled={staffLoading}
                            className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200/80 rounded-lg text-[11px] font-bold transition-all flex items-center gap-1 cursor-pointer shadow-2xs"
                            title="Edit Email, Nama, Role & Hak Akses"
                          >
                          ...
                          <button
                            ...
                            disabled={staffLoading}
                            className="px-2.5 py-1 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200/80 rounded-lg text-[11px] font-bold transition-all flex items-center gap-1 cursor-pointer shadow-2xs"
                            title="Hapus Hak Akses Email Staff Ini"
                          >
        ...
        {/* Edit Staff Profile Modal Overlay */}
        {editingStaff && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-xl border border-slate-100 max-w-md w-full p-5 space-y-4 animate-in fade-in zoom-in-95">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-800 text-sm">Edit Hak Akses Staff</h3>
                    <p className="text-[11px] text-slate-400">Ubah konfigurasi email, role, atau PIN untuk {editingStaff.name}</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <form onSubmit={handleUpdateStaffSubmit} className="space-y-3 text-xs">
                <div>
                  <label className="font-bold text-slate-600 block mb-1">Nama Lengkap Staff</label>
                  <input
                    type="text"
                    required
                    value={editName}
                    onChange={(e) => setEditName(e.target.value)}
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 bg-white focus:outline-none focus:border-indigo-500 font-medium text-xs"
                  />
                </div>

                <div>
                  <label className="font-bold text-slate-600 block mb-1">Alamat Email Google Staff</label>
                  <input
                    type="email"
                    required
                    value={editEmail}
                    onChange={(e) => setEditEmail(e.target.value)}
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 bg-white focus:outline-none focus:border-indigo-500 font-mono text-xs"
                  />
                  <span className="text-[10px] text-slate-400 mt-0.5 block">Alamat email ini digunakan untuk verifikasi Google Sign-In.</span>
                </div>

                <div>
                  <label className="font-bold text-slate-600 block mb-1">Role & Level Hak Akses</label>
                  <select
                    value={editRole}
                ...
                <div>
                  <label className="font-bold text-slate-600 block mb-1">4-Digit PIN Masuk Dapur</label>
                  <input
                    type="text"
                    required
                    maxLength={4}
                    ...
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 bg-white focus:outline-none focus:border-indigo-500 font-mono tracking-widest text-center text-sm font-bold"
                  />
                </div>
                ...
                <div className="pt-3 flex items-center justify-end gap-2 border-t border-slate-100">
                  <button
                    type="button"
                    onClick={() => setEditingStaff(null)}
                    className="px-3.5 py-2 rounded-lg text-slate-600 bg-slate-100 hover:bg-slate-200 font-bold transition-colors cursor-pointer"
                  >
                    Batal
                  </button>
                  <button
                    type="submit"
                    disabled={staffLoading}
                    className="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-bold flex items-center gap-1.5 shadow-sm transition-colors cursor-pointer"
                  >
                    {staffLoading ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <><Check className="w-3.5 h-3.5" /> Simpan Perubahan</>}
                  </button>
                </div>
              </form>
            </div>
